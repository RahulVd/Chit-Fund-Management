    ...
    public class AuctionService {
        ...
        // The furthest month allowed right now: (highest month already recorded, via
        // auction OR owner month) + 1. This lets a second (double chit) auction still
        // be added to the current month, while blocking anyone from skipping ahead.
        ...
        public Auction recordAuction(Long chitGroupId, Long winnerId,
            ...
            // NEW: enforce sequential months — can't jump ahead of the current open month
            int maxAllowedMonth = computeMaxAllowedMonth(chitGroupId);
            if (monthNumber > maxAllowedMonth) {
                throw new CustomException("Cannot record an auction for month " + monthNumber +
            ...
            long totalWinners = totalAuctionWinners + totalOwnerWinners;
